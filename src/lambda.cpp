...
void fe29_from_uint256_host(fe29_t& out, const uint64_t* in) {
    uint32_t words[8];
    for (int i = 0; i < 4; i++) {
        words[2*i] = (uint32_t)(in[i]);
        words[2*i+1] = (uint32_t)(in[i] >> 32);
    }
    out.v[0] = words[0] & 0x1FFFFFFF;
    out.v[1] = ((words[0] >> 29) | (words[1] << 3)) & 0x1FFFFFFF;
    out.v[2] = ((words[1] >> 26) | (words[2] << 6)) & 0x1FFFFFFF;
    out.v[3] = ((words[2] >> 23) | (words[3] << 9)) & 0x1FFFFFFF;
    out.v[4] = ((words[3] >> 20) | (words[4] << 12)) & 0x1FFFFFFF;
    out.v[5] = ((words[4] >> 17) | (words[5] << 15)) & 0x1FFFFFFF;
    out.v[6] = ((words[5] >> 14) | (words[6] << 18)) & 0x1FFFFFFF;
    out.v[7] = ((words[6] >> 11) | (words[7] << 21)) & 0x1FFFFFFF;
    out.v[8] = (words[7] >> 8) & 0x00FFFFFF;
}

void fe29_to_uint256_host(uint64_t* out, const fe29_t& in) {
    uint32_t words[8] = {0};
    words[0] = in.v[0] | (in.v[1] << 29);
    words[1] = (in.v[1] >> 3) | (in.v[2] << 26);
    words[2] = (in.v[2] >> 6) | (in.v[3] << 23);
    words[3] = (in.v[3] >> 9) | (in.v[4] << 20);
    words[4] = (in.v[4] >> 12) | (in.v[5] << 17);
    words[5] = (in.v[5] >> 15) | (in.v[6] << 14);
    words[6] = (in.v[6] >> 18) | (in.v[7] << 11);
    words[7] = (in.v[7] >> 21) | (in.v[8] << 8);
    for (int i = 0; i < 4; i++) {
        out[i] = ((uint64_t)words[2*i+1] << 32) | words[2*i];
    }
}
...
uint256_t lambda(std::string target_pubkey_hex, int key_range, int WALKERS, int DP_BITS, const std::string& snapoint_path, int snaptime_sec) {
    ...
    std::cout << CYAN << "[OpenCL] Using Device: " << RESET << PINK << default_device.getInfo<CL_DEVICE_NAME>() << RESET << std::endl;
    
    std::ifstream cl_file("src/kernel.cl");
    if (!cl_file.is_open()) {
        cl_file.open("kernel.cl");
        if (!cl_file.is_open()) {
            std::cerr << RED << "Error: Could not find kernel.cl (checked 'src/kernel.cl' and 'kernel.cl')" << RESET << std::endl;
            exit(1);
        }
    }
    std::string cl_string(std::istreambuf_iterator<char>(cl_file), (std::istreambuf_iterator<char>()));
    cl::Program program(context, cl_string);
    
    if (program.build({default_device}) != CL_SUCCESS) {
        std::cerr << RED << "OpenCL Build Error: " << program.getBuildInfo<CL_PROGRAM_BUILD_LOG>(default_device) << RESET << std::endl;
        return {0,0,0,0};
    }
    
    cl::Kernel kernel_walk(program, "kangaroo_walk");
    
    // Prepare StepTable
    std::vector<Step_cl> host_step_table(N_STEPS);
    for (uint32_t i = 0; i < N_STEPS; i++) {
        ...
        host_step_table[i].a[0] = a_arr[0];
        host_step_table[i].a[1] = a_arr[1];
        host_step_table[i].a[2] = a_arr[2];
        host_step_table[i].a[3] = a_arr[3];
    }
    
    cl::Buffer stepTableBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(Step_cl) * N_STEPS, host_step_table.data());
    
    // Prepare Walkers
    std::vector<WalkState_cl> host_walkers(WALKERS);
    for (int i = 0; i < WALKERS; i++) {
        ...
        host_walkers[i].b[0] = b_arr[0]; host_walkers[i].b[1] = b_arr[1]; host_walkers[i].b[2] = b_arr[2]; host_walkers[i].b[3] = b_arr[3];
        host_walkers[i].walk_id = w->walk_id;
        host_walkers[i].snapshot_steps = w->snapshot_steps;
        host_walkers[i].snapshot_x[0] = w->snapshot_x[0]; host_walkers[i].snapshot_x[1] = w->snapshot_x[1]; host_walkers[i].snapshot_x[2] = w->snapshot_x[2]; host_walkers[i].snapshot_x[3] = w->snapshot_x[3];
        host_walkers[i].state = 0;
    }
    
    cl::Buffer walkersBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, sizeof(WalkState_cl) * WALKERS, host_walkers.data());
    
    // DP Buffer (can hold up to 1000 DPs per launch)
    uint32_t dp_max_count = 1000;
    std::vector<DPEntry_cl> host_dp_buffer(dp_max_count);
    cl::Buffer dpBuffer(context, CL_MEM_WRITE_ONLY, sizeof(DPEntry_cl) * dp_max_count);
    cl::Buffer dpCountBuffer(context, CL_MEM_READ_WRITE, sizeof(uint32_t));
    ...
    kernel_walk.setArg(3, DP_BITS);
    kernel_walk.setArg(4, dpBuffer);
    ...
    auto worker = [&]() {
        try {
            while (search_in_progress.load(std::memory_order_acquire)) {
                ...
                queue.enqueueWriteBuffer(dpCountBuffer, CL_TRUE, 0, sizeof(uint32_t), &zero_count);
                
                cl_int err2 = queue.enqueueNDRangeKernel(kernel_walk, cl::NullRange, cl::NDRange(global_ws), cl::NDRange(local_ws));
                if (err2 != CL_SUCCESS) { std::cerr << "Error in enqueue: " << err2 << std::endl; exit(1); }
                
                cl_int err = queue.finish();
                if (err != CL_SUCCESS) { std::cerr << "Error in finish: " << err << std::endl; exit(1); }
                
                total_iters.fetch_add(WALKERS * kernel_steps, std::memory_order_relaxed);
                
                uint32_t dp_found = 0;
                queue.enqueueReadBuffer(dpCountBuffer, CL_TRUE, 0, sizeof(uint32_t), &dp_found);
                
                if (dp_found > dp_max_count) dp_found = dp_max_count; // safety bound
                
                if (dp_found > 0) {
                    queue.enqueueReadBuffer(dpBuffer, CL_TRUE, 0, sizeof(DPEntry_cl) * dp_found, host_dp_buffer.data());
                    
                    // Also we need to download the active Walkers buffer to reset the ones that finished!
                    queue.enqueueReadBuffer(walkersBuffer, CL_TRUE, 0, sizeof(WalkState_cl) * WALKERS, host_walkers.data());
                    
                    bool host_walkers_modified = false;
                    
                    for (uint32_t idx = 0; idx < dp_found; idx++) {
                        DPEntry_cl entry = host_dp_buffer[idx];
                        uint32_t w_id = entry.walk_id;
                        WalkState* w = &walkers_state[w_id];
                        
                        // Sync state
                        w->a.limbs[0] = entry.a[0]; w->a.limbs[1] = entry.a[1]; w->a.limbs[2] = entry.a[2]; w->a.limbs[3] = entry.a[3];
                        w->b.limbs[0] = entry.b[0]; w->b.limbs[1] = entry.b[1]; w->b.limbs[2] = entry.b[2]; w->b.limbs[3] = entry.b[3];
                        
                        DPEntry found_dp;
                        bool cl_collision = false;
                        dp_table.lazy_emplace_l(entry.x[0], [&](auto& bucket) {
                            auto& dps = bucket.second;
                            for (const auto& d_entry : dps) {
                                if (d_entry.x[1] == entry.x[1] && d_entry.x[2] == entry.x[2] && d_entry.x[3] == entry.x[3]) {
                                    bool same_state = (compare_uint256(w->a, d_entry.a) == 0) && (compare_uint256(w->b, d_entry.b) == 0);
                                    if (!same_state) {
                                        found_dp = d_entry;
                                        cl_collision = true;
                                    }
                            ...
                            if(!cl_collision) {
                                DPEntry new_entry;
                                new_entry.x[0] = entry.x[0]; new_entry.x[1] = entry.x[1]; new_entry.x[2] = entry.x[2]; new_entry.x[3] = entry.x[3];
                                new_entry.a = w->a;
                                new_entry.b = w->b;
                            ...
                            new_entry.a = w->a;
                            new_entry.b = w->b;
                            bucket(entry.x[0], std::vector<DPEntry>{new_entry});
                        } );
                        ...
                        if (cl_collision) {
                            ...
                            if (!scalarIsZero(db.limbs)) {
                                uint64_t res_k[4];
                                uint256_t res_k_256;
                                if (db.limbs[0] == 1 && db.limbs[1] == 0 && db.limbs[2] == 0 && db.limbs[3] == 0) res_k_256 = da;
                                else res_k_256 = sub_uint256(N, da);
                                memcpy(res_k, res_k_256.limbs, 32);
                                uint64_t neg_k[4]; scalarNeg(neg_k, res_k);
                                if (test_key(res_k)) solved = true;
                                if (!solved && test_key(neg_k)) solved = true;
                            }
                            if (!solved) {
                                ...
                                if (!scalarIsZero(db_sum.limbs)) {
                                    ...
                                    uint64_t neg_da[4]; memcpy(neg_da, da_sum.limbs, 32); scalarNeg(neg_da, neg_da);
                                    uint64_t res_k[4]; scalarMul(res_k, neg_da, inv_db_sum.limbs);
                                    uint64_t neg_k[4]; scalarNeg(neg_k, res_k);
                                    if (test_key(res_k)) solved = true;
                                    if (!solved && test_key(neg_k)) solved = true;
                                }
                            ...
                            if (solved) {
                                ...
                                host_walkers[w_id].b[0] = b_arr[0]; host_walkers[w_id].b[1] = b_arr[1]; host_walkers[w_id].b[2] = b_arr[2]; host_walkers[w_id].b[3] = b_arr[3];
                                host_walkers[w_id].state = 0; host_walkers[w_id].snapshot_steps = 0;
                                host_walkers_modified = true;
                            }
                        } else {
                            // reset walker
                            reset(w, w->walk_id % 2 == 0);
                    ...
                    for (int i = 0; i < WALKERS; i++) {
                        if (host_walkers[i].state == 2) { // cycled
                            ...
                            host_walkers[i].a[0] = a_arr[0]; host_walkers[i].a[1] = a_arr[1]; host_walkers[i].a[2] = a_arr[2]; host_walkers[i].a[3] = a_arr[3];
                            host_walkers[i].b[0] = b_arr[0]; host_walkers[i].b[1] = b_arr[1]; host_walkers[i].b[2] = b_arr[2]; host_walkers[i].b[3] = b_arr[3];
                            host_walkers[i].state = 0; host_walkers[i].snapshot_steps = 0;
                            total_cycles.fetch_add(1, std::memory_order_relaxed);
                            host_walkers_modified = true;
                        }
                    }
                    
                    if (host_walkers_modified) {
                        queue.enqueueWriteBuffer(walkersBuffer, CL_TRUE, 0, sizeof(WalkState_cl) * WALKERS, host_walkers.data());
                    }
                } // end if dp_found
            }
    ...
    std::thread progress_thread([&]() {
        ...
        while (search_in_progress.load(std::memory_order_acquire)) {
            ...
            if (now - last_print >= std::chrono::seconds(10)) {
                ...
                double ops_per_sec = (double)total_iters.load() / 10.0;
                double giga_ops = ops_per_sec / 1000000000.0;
                std::cout << CYAN << "\033[3A\r" << "\033[2KTotal Ops/10s: " << RESET << GREEN << total_iters.load() << " (" << std::fixed << std::setprecision(2) << giga_ops << " Giga Ops/s)" << RESET << "\n" << CYAN << "\033[2KSelf-Collision Cycles: " << RESET << GREEN << total_cycles.load() << RESET << "\n" << CYAN << "\033[2KCollision Probability: " << RESET << GREEN << std::fixed << std::setprecision(8) << (prob) << "...%" << RESET << CYAN << " | Snapoints: " << RESET << PINK << snapointStatus << RESET << "\n" << std::flush;
                last_print = now;
            }
