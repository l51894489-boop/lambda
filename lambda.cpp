...
uint256_t lambda(std::string target_pubkey_hex, int key_range, int WALKERS, int DP_BITS, const std::string& snapoint_path, int snaptime_sec) {
    ...
    for (uint32_t i = 0; i < N_STEPS; i++) {
        host_step_table[i].point = localStepTable[i].point;
        host_step_table[i].a = localStepTable[i].a;
        host_step_table[i].b = localStepTable[i].b;
    }
    HIP_CHECK(hipMemcpy(d_localStepTable, host_step_table.data(), N_STEPS * sizeof(StepLocal), hipMemcpyHostToDevice));
    
    uint32_t zero_count = 0;
    HIP_CHECK(hipMemcpy(d_dp_count, &zero_count, sizeof(uint32_t), hipMemcpyHostToDevice));
    
    std::thread progress_thread([&]() {
        const long double M = ldexpl(1.0L, key_range);
        unsigned long long last_ops = total_iters.load(std::memory_order_relaxed);
        while (search_in_progress.load(std::memory_order_acquire)) {
            auto now = std::chrono::steady_clock::now();
            if (now - last_print >= std::chrono::seconds(10)) {
                auto duration_sec = std::chrono::duration_cast<std::chrono::seconds>(now - last_print).count();
                if (duration_sec == 0) duration_sec = 1;
                unsigned long long current_ops = total_iters.load(std::memory_order_relaxed);
                unsigned long long ops_per_sec = (current_ops - last_ops) / duration_sec;
                ...
                long double prob = (1.0L - expl(-x)) * 100.0L;
    ...
    std::atomic<bool> snapoint_gpu_requested{false};
    ...
    if (!snapoint_path.empty() && snaptime_sec > 0) {
        snapoint_thread = std::thread([&]() {
            while (search_in_progress.load(std::memory_order_acquire)) {
                ...
                snapoint_gpu_requested.store(true, std::memory_order_release);
            }
        });
    }

    std::vector<DPResult> host_dp_buffer(buffer_size);
    ...
    while (search_in_progress.load(std::memory_order_acquire)) {
        ...
        uint32_t dp_count = 0;
        HIP_CHECK(hipMemcpy(&dp_count, d_dp_count, sizeof(uint32_t), hipMemcpyDeviceToHost));
        
        if (dp_count > 0) {
            uint32_t fetch_count = (dp_count > buffer_size) ? buffer_size : dp_count;
            HIP_CHECK(hipMemcpy(host_dp_buffer.data(), d_dp_buffer, fetch_count * sizeof(DPResult), hipMemcpyDeviceToHost));
            
            HIP_CHECK(hipMemcpy(d_dp_count, &zero_count, sizeof(uint32_t), hipMemcpyHostToDevice));
            
            bool solved = false;
            for (uint32_t i = 0; i < fetch_count; i++) {
                DPResult& dp_res = host_dp_buffer[i];
                DPEntry found_dp;
                bool cl = false;
                
                dp_table.lazy_emplace_l(dp_res.x[0], [&](auto& bucket) {
                    auto& dps = bucket.second;
                    for (const auto& entry : dps) {
                        if (entry.x[1] == dp_res.x[1] && entry.x[2] == dp_res.x[2] && entry.x[3] == dp_res.x[3]) {
                            bool same_state = (compare_uint256(dp_res.a, entry.a) == 0) && (compare_uint256(dp_res.b, entry.b) == 0);
                            if (!same_state) {
                                found_dp = entry;
                                cl = true;
                            }
                    ...
                    if(!cl) {
                        DPEntry new_entry;
                        std::memcpy(new_entry.x, dp_res.x, sizeof(new_entry.x));
                        new_entry.a = dp_res.a;
                        new_entry.b = dp_res.b;
                        dps.push_back(new_entry);
                    }
                }, [&](auto bucket) {
                    DPEntry entry;
                    std::memcpy(entry.x, dp_res.x, sizeof(entry.x));
                    entry.a = dp_res.a;
                    entry.b = dp_res.b;
                    bucket(dp_res.x[0], std::vector<DPEntry>{entry});
                });
                
                if (cl) {
                    uint256_t da, db;
                    if (compare_uint256(dp_res.a, found_dp.a) >= 0) da = sub_uint256(dp_res.a, found_dp.a);
                    else da = sub_uint256(N, sub_uint256(found_dp.a, dp_res.a));
                    if (compare_uint256(found_dp.b, dp_res.b) >= 0) db = sub_uint256(found_dp.b, dp_res.b);
                    else db = sub_uint256(N, sub_uint256(dp_res.b, found_dp.b));
                    ...
                    if (!solved) {
                        uint256_t da_sum = mod_add_N(dp_res.a, found_dp.a);
                        uint256_t db_sum = mod_add_N(dp_res.b, found_dp.b);
                    ...
                    if (solved) {
                        ...
                        // In a real implementation we would reset the specific walker on the GPU.
                        // Here we just record a cycle.
